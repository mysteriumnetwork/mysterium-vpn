...
void main() {
  ...
  setUp(() {
    ...
    store = VpnProtocolStore(mockLocalDB, mockAnalyticsStore, mockRemoteConfigStore);
  });
  ...
  group('getProtocol', () {
    ...
    test('returns openvpn if protocol picker is not available', () async {
      ...
      expect(protocol, ProtocolType.openvpn);
      expect(store.protocol, ProtocolType.openvpn);
      verifyNever(mockLocalDB.getProtocolType());
    });
  ...
  group('setProtocol', () {
    ...
    test('does nothing if protocol picker is not available', () async {
      when(mockRemoteConfigStore.isProtocolPickerAvailable).thenReturn(false);

      await store.setProtocol(ProtocolType.openvpn);
      verifyNever(mockLocalDB.setProtocolType(any));
