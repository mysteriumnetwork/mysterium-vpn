...
/// A centered empty-state with concentric "radar" rings behind a boxed [icon],
/// a [title], a [message], and an optional [action] below.
///
/// The rings' geometric center sits on the icon center and fade out toward the
/// edges. The top gap adapts to the viewport's remaining height: when there
/// isn't room for the full design-spec padding it shrinks (down to a small
...
/// resolved via `SliverLayoutBuilder`) so the top padding can clamp to the real
/// layout space. Avoid `LayoutBuilder` here — it doesn't support intrinsic
/// dimensions, which makes it incompatible with
...
class RadarEmptyState extends StatelessWidget {
  const RadarEmptyState({
    ...
    this.mobileTopPadding = _columnTopPaddingMobile,
    this.desktopTopPadding = _columnTopPaddingDesktop,
    super.key,
  });

  /// Glyph shown inside the boxed badge at the rings' center.
  final IconData icon;
  ...
  /// Optional action (e.g. a button) shown below the message.
  final Widget? action;

  /// Vertical space the widget is allowed to consume. When `null` the widget
  /// uses its design-spec desired padding (caller must not overflow the
  /// surrounding viewport).
  final double? availableHeight;

  /// Desired gap above the icon on mobile / desktop (clamped by
  /// [availableHeight] when space is tight).
  final double mobileTopPadding;
  final double desktopTopPadding;
  ...
  /// Default visible top padding above the icon — wider on desktop per the
  /// Figma. Overridable via [mobileTopPadding] / [desktopTopPadding].
  ...
  Widget build(BuildContext context) {
    ...
    final desiredTopPadding = isDesktop ? desktopTopPadding : mobileTopPadding;
    ...
    // [minTopPadding] so the icon never collides with the content above.
              ...
              children: [
                SizedBox(
                  ...
                  child: DecoratedBox(
                    ...
                    child: Icon(icon, size: 24, color: palette.iconPrimary),
                  ),
                ),
                ...
                if (action != null) ...[SizedBox(height: theme.spacing.xl), action!],
              ],
            ),
          ],
        ),
...
class _RadarRings extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ShaderMask(
    ...
    child: SizedBox.square(
      dimension: RadarEmptyState._ringsDiameter,
