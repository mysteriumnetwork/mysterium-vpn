...
class SlidingUpPanel extends StatefulWidget {
  const SlidingUpPanel({
    ...
    this.boxShadow = const <BoxShadow>[
      BoxShadow(blurRadius: 8, color: Color.fromRGBO(0, 0, 0, 0.25)),
    ],
    ...
    this.footer,
         ...
         panel != null || panelBuilder != null,
         'You must provide either a panel or a panelBuilder',
       ),
       assert(
         0 <= backdropOpacity && backdropOpacity <= 1.0,
         'Backdrop opacity must be between 0.0 and 1.0 inclusive',
       ),
       assert(
         snapPoint == null || 0 < snapPoint && snapPoint < 1.0,
         'Snap point must be between 0.0 and 1.0 exclusive',
       );
...
class SlidingUpPanelState extends State<SlidingUpPanel> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (widget.controller != null) {
      widget.controller!.panelState = this;
    }
  }
  ...
  Widget _gestureHandler({required Widget child}) {
    ...
    return Listener(
      ...
      onPointerMove: (PointerMoveEvent p) {
        ...
        // Determine the primary drag direction once movement exceeds touch slop.
        // Until determined, vertical panel handling is deferred so that horizontal
        // scrolling gestures (e.g. in horizontal lists) don't also trigger
        // vertical panel/scroll movement.
  ...
  //close the panel
  Future<void> _close() => _ac.fling(velocity: -1);

  //open the panel
  Future<void> _open() => _ac.fling();

  //hide the panel (completely offscreen)
  Future<void> _hide() => _ac.fling(velocity: -1).then((x) {
    setState(() {
  ...
  Future<void> _show() => _ac.fling(velocity: -1).then((x) {
    setState(() {
  ...
  Future<void> _animatePanelToPosition(
    ...
    assert(0.0 <= value && value <= 1.0, 'Panel position must be between 0.0 and 1.0 inclusive');
  ...
  Future<void> _animatePanelToSnapPoint({Duration? duration, Curve curve = Curves.linear}) {
    assert(widget.snapPoint != null, 'Snap point must not be null');
  ...
  // Allows programmatic scrolling by enabling the scroll flag that
  // would normally only be set by a user gesture.
...
class PanelController {
  ...
  SlidingUpPanelState get panelState => _panelState!;
  set panelState(SlidingUpPanelState panelState) {
  ...
  /// Enables scrolling on the panel's scroll controller so that
  /// programmatic scroll offsets are not reset by the panel's listener.
  void enableScrolling() {
    assert(isAttached, 'PanelController must be attached to a SlidingUpPanel');
    _panelState!._enableScrolling();
  }
}
