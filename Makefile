...
localizely-fetch:
	...
	trap 'status=$$?; \
	  if [ $$status -ne 0 ]; then \
	    cp "$$baseline"/intl_*.arb lib/l10n/ 2>/dev/null || true; \
	    echo "localizely-fetch failed (exit $$status) - ARBs rolled back." >&2; \
	  fi; \
	  rm -rf "$$baseline"; exit $$status' EXIT; \
...
check-layering:
	bash .github/scripts/check-layering.sh
